/*CHAD 2019_2020
 * autor: Pedro Silva
 * Programa que implementa a soma de 2 vectores usando OpenCL em GPU. v1 e v2 inicializados na GPU*/


// gcc -I /usr/local/cuda-10.1/include/ main.c -lm -lOpenCL -o opencl_vectorSum_2.o
#include <stdlib.h>
#include <stdio.h>
#include <CL/cl.h>
#include <math.h>
...
int main()
{
  ...
  printf("\nOpenCL device selected: %s",device_name);
	...
	FILE *fp=fopen("vectorSum_2.cl", "rb");
	if(fp==NULL)
	{
		printf("\nERROR: fopen(%s)", "vectorSum_2.cl");
		printf("\n\n");
		return 0;
	}
	else
	...
	cl_kernel kernel;
	kernel=clCreateKernel(program, "vectorSum_2", &cl_error);
	if(cl_error!=CL_SUCCESS)
	{
	...
	int *host_buffer_v1=(int *)malloc(vec_size_bytes);
    int *host_buffer_v2=(int *)malloc(vec_size_bytes);
    int *host_buffer_v3=(int *)malloc(vec_size_bytes);
    ...
    printf("host_buffer_v1[100] = %i\thost_buffer_v1[100] = %i\thost_buffer_v3[100] = %i\t.\n", host_buffer_v1[100], host_buffer_v2[100], host_buffer_v3[100]);*/
	cl_mem device_buffer_v1=clCreateBuffer(context, CL_MEM_READ_WRITE, vec_size_bytes, NULL, &cl_error);
	if(cl_error!=CL_SUCCESS)
	{
    ...
    cl_mem device_buffer_v2=clCreateBuffer(context, CL_MEM_READ_WRITE, vec_size_bytes, NULL, &cl_error);
	if(cl_error!=CL_SUCCESS)
	{
		printf("\nERROR: clCreateBuffer device_buffer; ERROR code:%i", cl_error);
		printf("\n\n");
	...
	cl_mem device_buffer_v3=clCreateBuffer(context, CL_MEM_WRITE_ONLY, vec_size_bytes, NULL, &cl_error);
	if(cl_error!=CL_SUCCESS)
	{
		printf("\nERROR: clCreateBuffer device_buffer; ERROR code:%i", cl_error);
		printf("\n\n");
	...
	cl_error=clSetKernelArg(kernel, 0, sizeof(cl_mem), &device_buffer_v1);
	if(cl_error!=CL_SUCCESS)
	{
		printf("\nERROR: clSetKernelArg device_buffer_v1; ERROR code:%i", cl_error);
		printf("\n\n");
		return 0;
	}
	cl_error=clSetKernelArg(kernel, 1, sizeof(cl_mem), &device_buffer_v2);
	if(cl_error!=CL_SUCCESS)
	{
		printf("\nERROR: clSetKernelArg device_buffer_v2; ERROR code:%i", cl_error);
		printf("\n\n");
		return 0;
	}
    cl_error=clSetKernelArg(kernel, 2, sizeof(cl_mem), &device_buffer_v3);
	if(cl_error!=CL_SUCCESS)
	{
		printf("\nERROR: clSetKernelArg device_buffer_v3; ERROR code:%i", cl_error);
		printf("\n\n");
		return 0;
	}
	...
	/*cl_error = clEnqueueWriteBuffer(command_queue, device_buffer_v1, CL_FALSE, 0, sizeof(int) * N, host_buffer_v1, 0, NULL, NULL);
    if(cl_error!=CL_SUCCESS)
	{
		printf("\nERROR: clEnqueueWriteBuffer host_buffer_v1 -> device_buffer_v1; ERROR code:%i", cl_error);
		printf("\n\n");
		return 0;
	}
		cl_error = clEnqueueWriteBuffer(command_queue, device_buffer_v2, CL_FALSE, 0, sizeof(int) * N, host_buffer_v2, 0, NULL, NULL);
    if(cl_error!=CL_SUCCESS)
	{
		printf("\nERROR: clEnqueueWriteBuffer host_buffer_v2 -> device_buffer_v2; ERROR code:%i", cl_error);
		printf("\n\n");
		return 0;
	}*/
	...
	cl_error=clEnqueueReadBuffer(command_queue, device_buffer_v3, CL_TRUE, 0, vec_size_bytes, host_buffer_v3, 0, NULL, NULL);
	if(cl_error!=CL_SUCCESS)
	{
	...
	// DEBUG:
	printf("\n\nDEBUG:");
	printf("\nhost_buffer_v3[0]=%i | host_buffer_v3[1]=%i | host_buffer_v3[%i]=%i", host_buffer_v3[0], host_buffer_v3[1], N-1, host_buffer_v3[N-1]);
  ...
  printf("\n\nElapsed time CLOCK_MONOTONIC (ms): %f", (t1_ms-t0_ms));
	...
	clReleaseMemObject(device_buffer_v1);
    clReleaseMemObject(device_buffer_v2);
    clReleaseMemObject(device_buffer_v3);
