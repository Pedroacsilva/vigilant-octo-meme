...
%a) Considere um sinal de fala guardado em formato WAV com frequência de amostragem de
% fs=20000 Hz (por exemplo, PT3.wav). Identifique os fatores de expansão, L, e de decimação, M
% (devem ser primos entre si).
fs_in = 20000;
fs_out = 48000;
fs_out_fac = factor(fs_out);
fs_in_fac = factor(fs_in);
%escolhendo os fatores nao comuns entre os dois vetores verificamos que
%o produto dos fatores não comuns é para fs_out: 2^5*5 e fs_in: 3*7^2
L = 2^5*5;
M = 3*7^2;
%b) Projete um filtro FIR com ordem 3200 (comprimento Nh=3201; atraso de 1600 amostras) com o
...
% c)Faça a implementação direta do sistema, usando as variáveis xe (sinal x expandido por L); xh
...
%d)Faça um plot do sinal x e xe em simultâneo, os pontos de x com asteriscos e os de xe com linhas.
fig = figure
...
%e)Faça um plot do sinal xh e y em simultâneo, os pontos de y com asteriscos e os de xh com linhas.
fig = figure
...
%f)vamos ver Nx amostras do sinal de entrada e definir 3 vetores de tempo em
...
fig = figure;
plot(t1,x(1:Nx),'rs',t2,xh(1:Nx*L),t3,y(1:Ny),'go')
grid, legend({'Sinal entrada','Sinal interpolado','Sinal de Saida'})
title('Sinal de entrada, interpolado e de saida, com atraso de 10 amostras')
print(fig , '-dpng', 'Imagens/atrasonossinais')

%g) Foram feitas 3200 multiplicacoes e acumulacoes para cada amostra do
%sinal de entrada
